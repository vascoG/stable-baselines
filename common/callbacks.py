...
class EvalCallbackWithTBRunningAverage(EventCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            # Log scalar value
            summary = tf.Summary(value=[tf.Summary.Value(tag=f'episode_reward/validation_reward{self.name}', simple_value=mean_reward)])
            self.locals['writer'].add_summary(summary, self.num_timesteps)

            summary = tf.Summary(value=[tf.Summary.Value(tag=f'episode_reward/validation_length{self.name}', simple_value=mean_ep_length)])
            self.locals['writer'].add_summary(summary, self.num_timesteps)
